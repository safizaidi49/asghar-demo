...
type CartItem = {
  ...
  price: number;
  qty: number;
  attrs?: string; // e.g. "Color: Denim Blue • 3 Cushions"
};
...
// Demo: placeholder cart (replace with your store state later)
...
export default function CheckoutPage() {
  ...
  // coupon demo (SAVE5 => 5% off)
  const [coupon, setCoupon] = useState("");
  const [couponOk, setCouponOk] = useState<null | { code: string; pct: number }>(
    null
  );
  ...
  function submitOrder(e: React.FormEvent) {
    ...
    // 👉 Here you’ll switch by `pay` and call your gateway
    // - Tabby / Tamara (BNPL) init
    // - CCAvenue redirect/init
    // - Create order in your backend, then redirect to success page
    console.log({
      payload: {
        customer: f,
        payment: pay,
        coupon: couponOk,
        items: cart,
        amounts: { subtotal, discount, delivery, total },
      },
    });

    alert(
      `Order created (demo).\nPayment: ${pay}\nTotal: ${formatAED(total)}\n\nReplace the submit handler with your real API calls (Tabby, Tamara, CCAvenue).`
    );
  }

  return (
    <div className="checkout-wrap">
      {/* Optional breadcrumb banner */}
          ...
          <section className="checkout-main">
            ...
            <div className="af-card">
              ...
              <div className="af-row">
                ...
                <div className="coupon-bar">
                  ...
                  />
                  <button className="af-btn af-btn--primary" onClick={applyCoupon}>
                    Apply
                  </button>
                </div>
                {couponOk ? (
                  <div className="af-help af-success">
                    Coupon <b>{couponOk.code}</b> applied: −{couponOk.pct}%.
                  </div>
                ) : coupon ? (
              ...
              <div className="af-row af-grid-2">
                ...
                <div className="af-field">
                  ...
                  <input
                    className="af-input"
                    value={f.address1}
                    onChange={(e) => setF({ ...f, address1: e.target.value })}
            ...
            <div className="af-card">
              ...
              <div className="payment-list">
                <label className="payment-item">
                  <input
                    ...
                    name="pay"
                    checked={pay === "tabby"}
                    onChange={() => setPay("tabby")}
                  />
                  {/* If you have logos: <Image src="/media/ICONS/tabby.svg" alt="" width={60} height={18}/> */}
                  <span>
                    Pay in 4 with <b>Tabby</b>{" "}
                    <span className="payment-note">No interest, no fees</span>
                  </span>
                </label>
                ...
                <label className="payment-item">
                  <input
                    ...
                    name="pay"
                    checked={pay === "tamara"}
                    onChange={() => setPay("tamara")}
                  />
                  <span>
                    Split in 4 with <b>Tamara</b>{" "}
                    <span className="payment-note">No interest, no fees</span>
                  </span>
                </label>
                ...
                <label className="payment-item">
                  <input
                    ...
                    className="payment-radio"
                    name="pay"
                    checked={pay === "ccavenue"}
                    onChange={() => setPay("ccavenue")}
                  />
                  <span>
                    <b>CCAvenue</b>{" "}
                    <span className="payment-note">Visa / MasterCard / AMEX</span>
                  </span>
                </label>
              </div>
              ...
              <label className="af-check">
                ...
                <span>
                  I agree to the{" "}
                  <Link href="/terms" className="af-link">
                    Terms & Conditions
                  </Link>{" "}
                  and{" "}
                  <Link href="/privacy" className="af-link">
                    Privacy Policy
                  </Link>
                  .
                </span>
            ...
            {/* SEO small links (optional) */}
            <div className="af-mini-links">
              <Link href="#">Asghar Furniture – Dubai</Link>
              <Link href="#">Asghar Furniture – Abu Dhabi</Link>
              <Link href="#">Asghar Furniture – Dubai JVC</Link>
              <Link href="#">Asghar Furniture – Ajman</Link>
            </div>
          </section>
          ...
          <aside className="checkout-aside summary">
            <div className="af-card">
              ...
              <div className="summary-list">
                ...
                <div className="summary-row">
                  <span>Delivery</span>
                  <span>{delivery === 0 ? "Free" : formatAED(delivery)}</span>
                </div>
                {discount > 0 && (
                  <div className="summary-row">
              ...
              {/* Installments information (visual only) */}
              <div className="af-card af-card--muted" style={{ marginTop: 12 }}>
                <div className="af-help">
                  As low as <b>{formatAED(total / 10)}</b>/month with Tabby over
                  10 months (illustrative).
                </div>
              </div>
            </div>
          </aside>
