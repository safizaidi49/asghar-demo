...
export default function CheckoutPage() {
  ...
  // signin/signup local state (front-end only)
  ...
  function submitOrder(e: React.FormEvent) {
    e.preventDefault();
    // guest/normal flow – basic requireds
    if (!emailOK(f.email) || !f.first || !phoneOK(f.phone) || !f.city || !f.address1) {
  ...
  function doSignIn(e: React.FormEvent) {
    e.preventDefault();
    if (!emailOK(loginEmail) || loginPwd.length < 8) {
      alert("Enter a valid email and password (8+ chars).");
      return;
    }
    // Demo: “log in” and pre-fill email into checkout; then switch to guest form
    setF((p) => ({ ...p, email: loginEmail }));
    setMode("guest");
    alert("Signed in (demo). Continue with checkout.");
  }

  function doSignUp(e: React.FormEvent) {
    e.preventDefault();
    if (regName.trim().length < 2 || !emailOK(regEmail) || regPwd.length < 8 || regPwd !== regPwd2) {
      alert("Please complete all fields correctly. Passwords must match.");
      return;
    }
    // Demo: “account created” and pre-fill email + first name; then switch to guest form
    setF((p) => ({ ...p, email: regEmail, first: regName.split(" ")[0] || regName }));
    setMode("guest");
    alert("Account created (demo). Continue with checkout.");
  }
      ...
      <div className="checkout-container">
        <div className="checkout-grid">
          ...
          <section className="checkout-main">
            {/* Mode tabs */}
            <div className="af-card">
              <div className="mode-tabs" role="tablist" aria-label="Checkout mode">
                <button
                ...
                <button
                  className={`mode-tab ${mode === "signin" ? "is-active" : ""}`}
                ...
                <button
                  className={`mode-tab ${mode === "signup" ? "is-active" : ""}`}
            ...
            {mode === "signin" && (
              <div className="af-card">
                <h2 className="af-h2">Sign in</h2>
                <form className="af-row" onSubmit={doSignIn} noValidate>
                  <div className="af-field">
                  ...
                  <div className="af-action-row">
                    <button className="af-btn af-btn--primary" type="submit">Sign in</button>
                    <button type="button" className="af-btn" onClick={() => setMode("signup")}>
                      Create an account
                    </button>
                  </div>
                </form>
            ...
            {mode === "signup" && (
              <div className="af-card">
                <h2 className="af-h2">Create your account</h2>
                <form className="af-row af-grid-2" onSubmit={doSignUp} noValidate>
                  <div className="af-field" style={{ gridColumn: "1 / -1" }}>
                    <label className="af-label">Full name *</label>
                    <input className="af-input" value={regName} onChange={(e) => setRegName(e.target.value)} placeholder="First Last" />
                  </div>
                  <div className="af-field" style={{ gridColumn: "1 / -1" }}>
                    <label className="af-label">Email *</label>
                    <input className="af-input" type="email" value={regEmail} onChange={(e) => setRegEmail(e.target.value)} placeholder="name@email.com" />
                  </div>
                  <div className="af-field">
                    <label className="af-label">Password *</label>
                    <input className="af-input" type="password" value={regPwd} onChange={(e) => setRegPwd(e.target.value)} placeholder="Minimum 8 characters" />
                  </div>
                  <div className="af-field">
                    <label className="af-label">Confirm password *</label>
                    <input className="af-input" type="password" value={regPwd2} onChange={(e) => setRegPwd2(e.target.value)} placeholder="Re-enter password" />
                  </div>
                  <div className="af-action-row" style={{ gridColumn: "1 / -1" }}>
                    <button className="af-btn af-btn--primary" type="submit">Create account</button>
                    <button type="button" className="af-btn" onClick={() => setMode("signin")}>
              ...
              <>
                ...
                <div className="af-card">
                  ...
                  <label className="af-check">
                    <input type="checkbox" checked={agree} onChange={() => setAgree(v => !v)} />
                    <span>
                      I agree to the <Link href="/terms-and-conditions" className="af-link">Terms & Conditions</Link> and{" "}
                      <Link href="/privacy-policy" className="af-link">Privacy Policy</Link>.
                    </span>
                  </label>
